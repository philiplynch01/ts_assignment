...
import cd_utils  # from the Fawaz CD diagram script
...
def plot_diff_bars(df: pd.DataFrame,
    ...
    for ax, diff_col, model, colour_base in [
        ...
        ax.set_yticks([])
        ax.set_xlabel(r"$\Delta$ Accuracy (ours $-$ reported)", fontsize=6)
        ax.set_title(f"{model}: ours vs. reported", fontsize=8)
...
def plot_critical_difference(df: pd.DataFrame, 
    ...
    """
    Converts a wide-format DataFrame (models as columns) into the long-format
    required by the CD diagram implementation and generates the plot.
    """
    # 1. Prepare the Output Directory
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # 2. Transform Data: Wide to Long format
    # We assume the index of your DF represents the dataset names/IDs
    df_long = df.reset_index().rename(columns={'index': 'dataset_name'})
    
    # Melt the dataframe: 
    # From: [dataset_name, ModelA, ModelB...] 
    # To:   [dataset_name, classifier_name, accuracy]
    df_perf = df_long.melt(
        id_vars=['dataset_name'], 
        var_name='classifier_name', 
        value_name='accuracy'
    )

    # 3. Call the external CD diagram logic
    # Note: I've modified the call to handle the filename properly 
    # within the context of your existing draw_cd_diagram function
    print(f"Generating CD diagram for {len(df.columns)} models across {len(df)} datasets...")
    
    # We use the draw_cd_diagram logic from the Fawaz script
    cd_utils.draw_cd_diagram(
        df_perf=df_perf, 
        alpha=0.05, 
    ...
    # Move the default output to your specific filename if needed
    if os.path.exists('cd-diagram.png'):
        path = os.path.join(output_dir, filename)
        os.rename('cd-diagram.png', path)
...
def plot_accuracy_distributions(df: pd.DataFrame,
    ...
    for ax, my_col, rep_col, model, colour in [
        ...
        sns.kdeplot(df[my_col],  ax=ax, color=colour,    linewidth=1.8,
                    label="Ours",     fill=True, alpha=0.25, clip=(0, 1))
        sns.kdeplot(df[rep_col], ax=ax, color="#555555", linewidth=1.5,
                    label="Reported", fill=True, alpha=0.15, clip=(0, 1), linestyle="--")
...
def plot_combined_figure(df: pd.DataFrame,
    ...
    for ax, xcol, ycol, title in [
        (ax_rep, "hydra_reported", "mrsqm_reported", "(a) Reported"),
        (ax_my, "hydra", "mrsqm", "(b) Ours"),
    ]:
        x, y = df[xcol], df[ycol]
        lo = min(x.min(), y.min()) - 0.02
        hi = max(x.max(), y.max()) + 0.02
        dot_colour = [C_NEUTRAL] * len(x)
        dot_colour = np.array(dot_colour)
        dot_colour[x<y] = C_MRSQM
        dot_colour[x>y] = C_HYDRA
        ...
        ax.set_ylabel("MrSQM accuracy", fontsize=7)
        ax.set_xlabel("HYDRA accuracy", fontsize=7)
        ax.set_title(title, fontsize=8)
        ax.set_aspect("equal")
        ...
        above = (y > x).sum()
        below = (x > y).sum()
        ax.text(0.03, 0.97, f"MrSQM↑ {above}\nHYDRA ↑ {below}",
    ...
    ax_box.set_ylabel(r"$\Delta$ accuracy (ours $-$ reported)", fontsize=7)
